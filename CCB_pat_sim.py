...
def pat_sim_q(k_in_cen,OR,res_cut):
    ...
    for l in range(num):
        HKL=HKL_list[l,0:3]
        K_in_SL, K_out_SL=source_line_scan(k_in_cen,OR,HKL,rot_ang_step=0.05,rot_ang_range=1.5)
	...
	for x_p in x0_arry:
		for y_p in y0_arry:
			for z_p in z0_arry:
				...
				y0.append(y_p)
				z0.append(z_p)
				D.append(1)  ## binary model as the first try.
	x0 = np.array(x0)
	y0 = np.array(y0)
	z0 = np.array(z0)
	D = np.array(D)
...
def get_D(xtal_model_dict,k_in,delta_k_in):
	'''
...
def get_P(ref_image,k_in):
	...
	pu = ref_image[y_ind,x_ind]/1e6                     # the normalised pixel value of pupil shade for k_in. no mean is taken here.
	if (pu>2):
		P_value = pu
	else:
		P_value = 0
