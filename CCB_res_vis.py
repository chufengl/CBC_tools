...
class Res_File:
	...
	def plot_geom(self):
		...
		ax[0].plot(self.frame_list,1/np.array(self.cam_len_list))
		ax[0].set_title('Cam_len')
		ax[1].plot(self.frame_list,self.kosx_list)
		ax[1].set_title('kosx')
		ax[2].plot(self.frame_list,self.kosy_list)
		ax[2].set_title('kosy')
		i = 0
		while os.path.isfile('res_geom_'+str(i)+'.png'):
			i = i + 1
		
			
		plt.savefig('res_geom_'+str(i)+'.png')
	def plot_TG(self):
		fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(5,5))
		ax.plot(self.frame_list,self.ini_TG_list,'b',self.frame_list,self.final_TG_list,'g')
		ax.legend(['initial_TG','final_TG'])
		i = 0
		while os.path.isfile('res_TG_'+str(i)+'.png'):
			i = i + 1
		plt.savefig('res_TG_'+str(i)+'.png')
