...
Generated by CrystFEL'''
		Crystfel_out_put_arry = np.hstack((out_put_arry[:,1:2],out_put_arry[:,0:1],out_put_arry[:,2:3],out_put_arry[:,10:11],np.zeros((out_put_arry.shape[0],1)),out_put_arry[:,11:12]/np.sqrt(out_put_arry[:,3:4]),out_put_arry[:,3:4]))
	...
	def show_frame(self,frame_no,image_file='/gpfs/cfel/cxi/scratch/user/lichufen/CCB_ind/scan_corrected_00135.h5',h5path='/corrected_data/corrected_data',show_hkl=True):
		with h5py.File(image_file,'r') as im:
			img_arry = np.array(im[h5path][frame_no,:,:])
		with h5py.File('/gpfs/cfel/cxi/scratch/user/lichufen/CCB_ind/mask.h5','r') as m:
			mask = np.array(m['/data/data'])
	...
	def get_frame_norm_raw(self,frame_no,bins=200):
		...
		for r in f.reflection_obj_list:
			...
			if not np.isnan(r.reference_I):
				...
				q_vec = r.hkl*(1e10/np.array([21.02,16.00,24.54]))
				Lorf = CCB_pat_sim.get_Lorf(q_vec,k0)
				#Lorf = 1

				Int_n = Int_n + [p.inte/(r.reference_I*Lorf) for p in r.pixel_obj_list]
		f.k_in_x = k_in_x
		f.k_in_y = k_in_y
		f.Int_n = Int_n
	...
	def get_all_frames_norm(self):
		norm_all = []
		for frame_no in self.frame_arry:
			KX,KY,INT_n_mean = self.get_frame_norm_raw(frame_no)
	...
	def show_frame_norm(self,frame_no):
		...
		plt.draw()
		plt.title('frame_norm,frame_no: %d'%(f.frame_no))
		#plt.show()
...
class Reflection:
	...
	def get_inte_adj(self):
		if hasattr(self.pixel_obj_list[0], 'inte_adj'):
			...
			q_vec = self.hkl*(1e10/np.array([21.02,16.00,24.54]))
			Lorf = CCB_pat_sim.get_Lorf(q_vec,k0)
